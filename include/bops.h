...
{//This function reads PPI from Ppidata_file
    ...
    fin.close();
    // update mp
    mp.resize(Protein_set.size() + 1, vector<bool>(Protein_set.size() +1, false));
    for(auto& e: Current_ppi.interaction) {
        mp[e.proteinb][e.proteina] = true;
        mp[e.proteina][e.proteinb] = true;
    }
}
...
double coff(PPI& ppi) {
    int edge_count = 0;
    for(int i = 0; i < ppi.protein.size(); ++i) {
        for(int j = i + 1; j < ppi.protein.size(); ++j)  {
            if(mp[i][j]) {
                edge_count += 1;
            }
        }
    }
//    std::cout << (double)edge_count * 2 / (ppi.interaction.size() * (ppi.interaction.size() - 1)) << endl;
    return (double)edge_count * 2 / (ppi.interaction.size() * (ppi.interaction.size() - 1));
}
...
{//This function is to split huge ppi into small ppi
    ...
    if (Current_ppi.protein.size() <= MAXP)
    {
        ...
        return;
    } else if(Current_ppi.protein.size() > MAXP && Current_ppi.protein.size() <= 80) {   // 中等复合物
        if(coff(Current_ppi) >= 0.35) {
            Splitted_ppi.push_back(Current_ppi);
        }
    }
...
{//This function determines whether the connected subset is a protein complex and removes protein complexes that are too similar
    //
    if(Complex.protein.size() == 3) {
        for(int i = 0; i < 3; ++i) {
            for(int j = i + 1; j < 3; ++j) {
                if(!mp[Complex.protein[i]][Complex.protein[j]]) {
                    return;
                }
            }
        }
